...
MONTHLY_URL = "https://github.com/trending?since=monthly&spoken_language_code=en"
...
def extend_repo_list():
    with open("trending_repos.repo", mode="a+") as tf:
        repo_list = tf.readlines()
        logging.info(f"Trending Repos Contains {len(repo_list)} repos.")
        new_trending_repos = [repo for repo in get_trending_repos() if repo not in repo_list]
        logging.info(f"Found {len(new_trending_repos)} new english trending repos.")
